...
export function ProgramCard({ program }: { program: Program }) {
  ...
  return (
    ...
    >
      ...
      <h4 className="mt-3 text-base md:text-lg font-bold capitalize leading-tight text-heading line-clamp-2 min-h-[2.75rem]">{program.title}</h4>
      <p className="mt-0.5 line-clamp-1">{program.tagline}</p>
      <p className="mt-2 flex-1 line-clamp-3">{program.description}</p>
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-3xl border border-border bg-card shadow-soft transition-all hover:-translate-y-1 hover:shadow-card">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <h4 className="mt-2.5 text-base md:text-lg font-bold capitalize leading-tight text-heading line-clamp-2 min-h-[2.75rem]">{event.title}</h4>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <h4 className="mt-2.5 text-base md:text-lg font-bold capitalize leading-tight text-heading transition-colors group-hover:text-primary line-clamp-2 min-h-[2.75rem]">{blog.title}</h4>
...
export function StoryCard({ story }: { story: Story }) {
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-3xl border border-border bg-card shadow-soft transition-all hover:-translate-y-1 hover:shadow-card">
      ...
      <div className="flex flex-1 flex-col p-5">
        <h4 className="text-base md:text-lg font-bold capitalize leading-tight text-heading line-clamp-2 min-h-[2.75rem]">{story.headline}</h4>
        <p className="mt-1.5 flex-1 line-clamp-3">{story.summary}</p>
