...
const nodesData = [
  ...
  },
  {
    id: 2,
    name: "Healthcare",
    label: "Accessible Preventive Healthcare",
    icon: <HeartIcon />,
    left: "42.0%",
    top: "83.6%",
  ...
  },
  {
    ...
    label: "Vocational & Youth Upskilling",
  ...
  },
  {
    ...
    label: "Grassroots Startups & Micro-loans",
    icon: <LightbulbIcon />,
    left: "23.3%",
    top: "39.9%",
    color: "#0B4CA3"
  },
  {
    ...
    label: "Financial & Leadership Autonomy",
    icon: <PersonIcon />,
    left: "36.0%",
    top: "70.8%",
    color: "#0B4CA3"
  },
  {
    ...
    label: "Local Leadership & Assistance Networks",
    icon: <HandshakeIcon />,
    left: "43.5%",
    top: "46.3%",
    color: "#10B981" // Emerald accent
  },
  {
    id: 8,
    name: "Volunteering",
    label: "National Network of Change-Makers",
  ...
  },
  {
    ...
    label: "Digital Banking & Wealth Education",
...
// Local components for floating particle effect
function FloatingParticles() {
  ...
  useEffect(() => {
    // Generate static random attributes on mount
...
export default function Loader({ onComplete, mode = "full" }: LoaderProps) {
  const [stage, setStage] = useState(1);
  const [isFirstVisit, setIsFirstVisit] = useState(true);
  const [currentMessageIndex, setCurrentMessageIndex] = useState(0);

  // Check visit history on mount
  useEffect(() => {
    if (mode === "navigation") {
      setStage(8); // Navigation stage
      return;
    }

    const hasVisited = sessionStorage.getItem('disha_loader_viewed');
    const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    if (hasVisited || prefersReducedMotion) {
      setIsFirstVisit(false);
      setStage(7); // Jump straight to logo reveal
    } else {
  ...
  // Storyboard Timing Sequencer (First Visit)
  useEffect(() => {
    if (mode === "navigation") {
      const timer = setTimeout(() => {
        onComplete();
      }, 450); // Fast route transition
      return () => clearTimeout(timer);
    }

    if (!isFirstVisit) {
      // Refresh flow: Logo reveal is displayed, then complete
      const timer = setTimeout(() => {
        onComplete();
      }, 2500); // 2.5 seconds simple logo transition
      return () => clearTimeout(timer);
    }

    const timers: ReturnType<typeof setTimeout>[] = [];

    // Stage 1 -> 2: Spark draws path (0.8s)
    timers.push(setTimeout(() => setStage(2), 800));

    // Stage 2 -> 3: India outline drawn (2.0s)
    timers.push(setTimeout(() => setStage(3), 2000));

    // Stage 3 -> 4: Nodes light up (3.5s)
    timers.push(setTimeout(() => setStage(4), 3500));

    // Stage 4 -> 5: Nodes bloom into icons (5.0s)
    timers.push(setTimeout(() => setStage(5), 5000));

    // Stage 5 -> 6: Hope wave and rotating messages (6.2s)
    timers.push(setTimeout(() => setStage(6), 6200));

    // Stage 6 -> 7: Dissolve India & show Logo (7.2s)
    timers.push(setTimeout(() => {
      setStage(7);
      sessionStorage.setItem('disha_loader_viewed', 'true');
    }, 7200));

    // Stage 7 -> Complete (9.7s - Logo displayed for 2.5 seconds)
    timers.push(setTimeout(() => {
      onComplete();
    }, 9700));

    return () => timers.forEach(clearTimeout);
  }, [isFirstVisit, onComplete, mode]);

  // Rotate messages during Stage 6
  useEffect(() => {
    if (stage === 6) {
      const interval = setInterval(() => {
        setCurrentMessageIndex((prev) => (prev + 1) % rotatingMessages.length);
      }, 200); // Quick rotation during the hope wave sweep (1.0s duration)
      return () => clearInterval(interval);
    }
  }, [stage]);

  // Render quick route transition loader using only the logo ring animation of the final scene
  if (mode === "navigation" || stage === 8) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        ...
        style={{
          ...
          backgroundColor: 'rgba(0, 0, 0, 0.75)', // Dark theme backdrop
        ...
        >
          ...
          <motion.div
            ...
            style={{
              position: 'absolute',
              inset: 0,
              border: '2px dashed rgba(11, 76, 163, 0.5)',
              borderRadius: '50%'
            }}
          />
          {/* Pulsing inner ring */}
          ...
          />
          {/* Central logo container with optimal optical padding */}
          <div
            style={{
              position: 'absolute',
              inset: '12px',
              ...
              alignItems: 'center',
              justifyContent: 'center',
              boxShadow: '0 4px 18px rgba(11, 76, 163, 0.3), 0 0 0 1px rgba(11, 76, 163, 0.1)',
              overflow: 'hidden',
              padding: '9px'
            }}
          >
      ...
      >
        <AnimatePresence mode="wait">
          {stage < 7 ? (
            ...
            >
              {/* Camera Zoom effect applied to the SVG */}
              <motion.svg
                viewBox={`0 0 ${VIEW_WIDTH} ${VIEW_HEIGHT}`}
                style={{ width: '100%', height: '100%', overflow: 'visible' }}
                animate={stage >= 3 ? { scale: 1.05 } : { scale: 1.0 }}
                transition={{ duration: 5.0, ease: "easeOut" }}
              >
                {/* Glowing Base Border Path */}
                <motion.path
                  d={indiaPath}
                  fill="none"
                  stroke="#0B4CA3"
                  ...
                  transition={{
                    pathLength: { delay: 0.0, duration: 1.2, ease: "easeInOut" },
                    opacity: { duration: 0.2 }
                  }}
                />

                {/* Ambient Internal Glow of India Map */}
                {stage >= 3 && (
                  <motion.path
                    d={indiaPath}
                    fill="#0B4CA3"
                    initial={{ fillOpacity: 0 }}
                    animate={{ fillOpacity: 0.04 }}
              ...
              {/* Floating particles inside the map (Stage 4+) */}
              {stage >= 4 && <FloatingParticles />}

              {/* Nodes and Icons */}
              {stage >= 4 &&
                nodesData.map((node) => {
                  const isEmerald = node.color === '#10B981';
                  const hasBloomed = stage >= 5;
                  ...
                  return (
                    ...
                    >
                      {/* Node Core Indicator / Icon Bloom */}
                      <motion.div
                        className={isEmerald ? "glow-emerald" : "glow-orange"}
                        ...
                        transition={{
                          ...
                          stiffness: 150,
                          damping: 12,
                          delay: hasBloomed ? 0 : Math.random() * 0.4
                        }}
                        ...
                        style={{
                          ...
                          justifyContent: 'center',
                          borderRadius: '50%',
                          width: hasBloomed ? '36px' : '14px',
                          height: hasBloomed ? '36px' : '14px',
                          backgroundColor: hasBloomed ? '#FFFFFF' : (isEmerald ? '#10B981' : '#0B4CA3'),
                          border: hasBloomed ? `2px solid ${isEmerald ? '#10B981' : '#0B4CA3'}` : '2px solid #FFFFFF',
                          color: isEmerald ? '#10B981' : '#0B4CA3',
                          cursor: 'pointer',
                          transition: 'width 0.4s cubic-bezier(0.34, 1.56, 0.64, 1), height 0.4s cubic-bezier(0.34, 1.56, 0.64, 1), background-color 0.4s, border-color 0.4s'
                        }}
                      >
                        {hasBloomed && (
                          <motion.div
                      ...
                      {/* Tooltip Card - Glassmorphic Black with Blue/Green outline */}
                      {hasBloomed && (
                        <div
                          className="absolute left-1/2 -bottom-16 -translate-x-1/2 loader-tooltip z-30"
                          style={{
                            borderColor: isEmerald ? '#10B981' : '#0B4CA3',
                            boxShadow: `0 0 10px ${isEmerald ? 'rgba(16, 185, 129, 0.25)' : 'rgba(11, 76, 163, 0.25)'}`
                          }}
                        >
                          <span style={{ color: isEmerald ? '#10B981' : '#0B4CA3', fontSize: '11px', fontWeight: 700, letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>
                            {node.name}
                          </span>
                          <span style={{ color: '#111827', fontSize: '9px', whiteSpace: 'nowrap', marginTop: '2px', opacity: 0.85 }}>
                            {node.label}
                          </span>
                        </div>
                      )}
              ...
              {/* Hope Wave sweeping horizontally (Stage 6) */}
              {stage === 6 && (
                <motion.div
                  style={{
                    position: 'absolute',
            ...
            /* Stage 7: cinematic Logo and Tagline Reveal */
            <motion.div
              key="logo-reveal"
            ...
            >
              ...
              <div style={{ position: 'relative', width: '120px', height: '120px', marginBottom: '2rem' }}>
                ...
                />
                {/* Central white emblem badge with proportional padding */}
                <div
                  style={{
                    position: 'absolute',
                    inset: '16px',
                    ...
                    alignItems: 'center',
                    justifyContent: 'center',
                    boxShadow: '0 6px 24px rgba(11, 76, 163, 0.35), 0 0 0 1px rgba(11, 76, 163, 0.1)',
                    overflow: 'hidden',
                    padding: '14px'
                  }}
                >
              ...
              >
                DISHA <span style={{ color: '#0B4CA3' }}>FOR</span> INDIA
              </h1>
...
export { Loader as LoadingScreen };
